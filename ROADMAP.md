...
- [ ] **Executable First Light extension** (INTENSIVE / Developer): replace the invitation's prose-only next step with a complete, safe, ready-to-POST `/api/v1/batch` body for a small addition adjacent to (not overwriting) the frame. Include an explicit builder placeholder, exact observation region, and equivalent MCP `build` arguments; expose the same payload in `build_something` and agent guides. Add contract tests that every coordinate is in bounds, outside the 84 seeded cells, and accepted by the batch validator. Do not place it during verification.
