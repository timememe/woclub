...
async function commitOps(kv, ops, preview = false, protectExisting = false) {
  const meta = await readMeta(kv);
  ...
  if (protectExisting) {
    ...
    for (const result of ordered) {
      const id = result.x + ":" + result.y + ":" + result.z;
      if (result.ok && (result.op === "place" || result.removed) && cells.get(id).before) {
        conflicts.set(id, {x: result.x, y: result.y, z: result.z});
      }
    }
    if (conflicts.size) return {ok: false, error: "existing_cells_conflict", protect_existing: true,
      conflicts: [...conflicts.values()], ...(preview ? {preview: true,
        note: "Read-only KV estimate, not a reservation or authoritative emptiness check. No operations applied."} : {})};
  }
  if (preview) {
...
async function mutateWorld(env, action, payload) {
  if (!env.WORLD_COORDINATOR) {
    ...
    return action === "clear" ? clearBuilder(env.METRICS, payload.builder) : commitOps(env.METRICS, payload.ops, false, payload.protect_existing);
  }
  try {
...
async function handleMcpRpc(request, env, context) {
  ...
  if (name === "build" || name === "preview_build") {
    ...
    if (args.protect_existing !== undefined && typeof args.protect_existing !== "boolean") return mcpResponse(message.id, mcpToolResult({error: "invalid_protect_existing"}, true));
    const check = validateOps(args.ops, args.builder);
    if (check.error) return mcpResponse(message.id, mcpToolResult(check, true));
    if (name === "preview_build") {
      const result = await commitOps(kv, check.ops, true, args.protect_existing);
      return mcpResponse(message.id, mcpToolResult(result, Boolean(result.error)));
    }
    const outcome = await mutateWorld(env, "ops", {ops: check.ops, request_id: args.request_id, protect_existing: args.protect_existing});
    if (outcome.error) return mcpResponse(message.id, mcpToolResult(batchResponse(outcome), true));
...
- Protect existing cubes: set protect_existing:true in batch/build and preview/preview_build. Accepted edits to initially occupied cells atomically reject the whole batch with existing_cells_conflict (409 / MCP tool error); preview is only a KV estimate.
...
- Optional protect_existing:true on batch/build and preview/preview_build protects cells occupied before the batch. Accepted placements (even same type/handle) and effective removals on these cells reject the WHOLE batch: existing_cells_conflict, HTTP 409 / MCP tool error, conflicts:[{x,y,z}] (at most 512 unique coordinates), no world/count/activity changes. Invalid operations alone do not conflict. Ordered edits on initially empty cells remain allowed; remove-then-place cannot bypass protection. Default false preserves partial rejection and replacement semantics. Preview checks eventual KV only, never reserves space or proves authoritative emptiness. Keyed rejections are durably retained/replayed for 24 hours with receipt lookup status:rejected; committed_at is the historical outcome-recording time even for a rejected batch. A 409 is a known constraint outcome; a timeout/503 is uncertain and requires reconciliation.
...
2. Explicitly POST {request_id, protect_existing?, builder?, ops} to /api/v1/batch, or call build with those arguments. The response includes receipt {request_id, committed_at, expires_at} and replayed:false, plus the original summary/results.
3. After an uncertain response, GET /api/v1/receipts/{request_id} or call get_build_receipt {request_id}. The authoritative coordinator returns status:committed or status:rejected with the historical outcome, or status:unknown (absent OR expired). A storage/network failure is unavailable, never unknown. All receipt responses are no-store.
4. Within retention, resubmitting that ID and the same normalized ordered operations and effective protect_existing flag returns the original receipt/summary/results with replayed:true and no world mutation, including after another builder changes the cells. JSON property order, ignored fields and equivalent effective builder defaults do not matter. Omitted protect_existing equals false. A changed execution payload returns request_id_conflict (HTTP 409 / MCP tool error). Receipt lookup and replay work even during a KV projection outage.
...
const openapi = {
  ...
  paths: {
    ...
    "/api/v1/receipts/{request_id}": {get: {summary: "Look up an authoritative historical batch receipt (public by ID, no-store)", parameters: [{name: "request_id", in: "path", required: true, schema: requestIdSchema}], responses: {"200": {description: "status:committed or status:rejected with request_id, committed_at, expires_at and original outcome; or status:unknown (absent OR expired, never proof of non-commit). Retained for 24 hours; not current cell occupancy."}, "400": {description: "invalid_request_id"}, "503": {description: "world_storage_unavailable, never reported as unknown"}}}},
    "/api/v1/batch": { post: { summary: "Apply a chain of place/remove ops", requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["ops"], properties: { protect_existing: {type: "boolean", default: false, description: "Atomically reject accepted edits to cells occupied before the batch; preview is only a KV estimate."}, request_id: requestIdSchema, builder: { type: "string" }, ops: { type: "array", minItems: 1, maxItems: MAX_BATCH_OPS, items: { type: "object", required: ["x", "y", "z"], properties: { op: { type: "string", enum: ["place", "remove"] }, x: { type: "integer" }, y: { type: "integer" }, z: { type: "integer" }, type: { type: "string", enum: TYPES }, builder: { type: "string" } } } } } } } } }, responses: { "200": { description: "Per-op results and a summary; keyed writes add receipt {request_id, committed_at, expires_at} and replayed. Retention 24 hours, max 10,000 retained IDs. Matching normalized replays do not mutate; after expiry reusing the ID may execute again." }, "400": { description: "invalid_request_id, invalid_protect_existing, invalid_batch or invalid_op" }, "409": { description: "existing_cells_conflict with at most 512 conflicts:{x,y,z}, no world changes; or request_id_conflict: retained ID with different operations/protect_existing" }, "503": { description: "receipt_capacity or world_storage_unavailable; reconcile before retrying" }, "413": { description: `Body exceeds ${BULK_BODY_BYTES} bytes` } } } },
    "/api/v1/preview": { post: { summary: "Preview a batch without persistent writes; estimate only, not a reservation", requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["ops"], properties: { protect_existing: {type: "boolean", default: false, description: "Atomically reject accepted edits to cells occupied before the batch; preview is only a KV estimate."}, request_id: requestIdSchema, builder: { type: "string" }, ops: { type: "array", minItems: 1, maxItems: MAX_BATCH_OPS, items: { type: "object", required: ["x", "y", "z"], properties: { op: { type: "string", enum: ["place", "remove"] }, x: { type: "integer" }, y: { type: "integer" }, z: { type: "integer" }, type: { type: "string", enum: TYPES }, builder: { type: "string" } } } } } } } } }, responses: { "200": { description: "Per-op results, accepted count, summary, inclusive affected bounds and up to 512 before/after cells" }, "400": { description: "invalid_request_id, invalid_protect_existing, invalid_batch or invalid_op" }, "409": {description: "existing_cells_conflict with up to 512 conflicting coordinates, based on eventual KV; no writes"}, "413": { description: `Body exceeds ${BULK_BODY_BYTES} bytes` } } } },
  ...
  async fetch(request, env = {}, context = {}) {
    ...
    if (request.method === "POST" && url.pathname.startsWith("/api/v1/")) {
      ...
      if (route === "batch" || route === "preview") {
        ...
        if (body?.protect_existing !== undefined && typeof body.protect_existing !== "boolean") return json({error: "invalid_protect_existing"}, 400);
        const check = validateOps(body?.ops, body?.builder);
        if (check.error) return json(check, 400);
        if (route === "preview") {
          const result = await commitOps(kv, check.ops, true, body.protect_existing);
          return json(result, result.error ? 409 : 200, {"cache-control": "no-store"});
        }
        const outcome = await mutateWorld(env, "ops", {ops: check.ops, request_id: body.request_id, protect_existing: body.protect_existing});
        if (outcome.error) return json(batchResponse(outcome), 409, {"cache-control": "no-store"});
        const builder = check.ops.find((op) => op.builder)?.builder || null;
        context.waitUntil?.(recordUsage(kv, request, "batch", { cubes_added: outcome.replayed ? 0 : outcome.added, cubes_removed: outcome.replayed ? 0 : outcome.removed, builder }));
