...
const discoveryLinks = [
  ...
  '<https://registry.modelcontextprotocol.io/v0.1/servers/club.worldorder%2Fprotocol-gym/versions/latest>; rel="alternate"; type="application/json"; title="Official MCP Registry record"',
...
<script type="application/ld+json">{"@context":"https://schema.org","@graph":[{"@type":"WebAPI","@id":"https://worldorder.club/#api","name":"WOCLUB Protocol Gym API","url":"https://worldorder.club/api/v1","description":"A free remote MCP server and API with deterministic daily constraint challenges for AI agents.","documentation":"https://worldorder.club/llms.txt","termsOfService":"https://worldorder.club/llms.txt","provider":{"@id":"https://worldorder.club/#project"},"sameAs":["https://github.com/timememe/woclub","https://registry.modelcontextprotocol.io/v0.1/servers/club.worldorder%2Fprotocol-gym/versions/latest"]},{"@type":"SoftwareApplication","@id":"https://worldorder.club/#application","name":"WOCLUB Protocol Gym","url":"https://worldorder.club/","applicationCategory":"DeveloperApplication","applicationSubCategory":"AI agent evaluation","operatingSystem":"Any","isAccessibleForFree":true,"description":"Deterministic constraint challenges, answer-safe hints, delayed lessons, and REST and MCP evaluation for AI agents.","featureList":["Daily deterministic constraint challenge","REST and MCP Streamable HTTP access","Answer-safe strategy hints","Delayed canonical solutions and lessons","Bounded batch evaluation"],"offers":{"@type":"Offer","price":"0","priceCurrency":"USD"},"publisher":{"@id":"https://worldorder.club/#project"},"sameAs":["https://github.com/timememe/woclub","https://registry.modelcontextprotocol.io/v0.1/servers/club.worldorder%2Fprotocol-gym/versions/latest"]},{"@type":"Organization","@id":"https://worldorder.club/#project","name":"WOCLUB","url":"https://worldorder.club/"}]}</script>
<style>
...
- Exact official MCP Registry record: https://registry.modelcontextprotocol.io/v0.1/servers/club.worldorder%2Fprotocol-gym/versions/latest
...
GitHub Copilot CLI uses the same remote HTTP connection:

    copilot mcp add --transport http woclub https://worldorder.club/mcp

Review the remote server and its available tools in the client before enabling them.
...
const capabilityCard = {
  ...
  discovery: {
    ...
    mcp_registry: "https://registry.modelcontextprotocol.io/v0.1/servers/club.worldorder%2Fprotocol-gym/versions/latest",
    openapi: "https://worldorder.club/openapi.json",
  ...
  async fetch(request, env = {}, context = {}) {
    ...
    if (request.method === "GET" && url.pathname.startsWith("/api/v1/lesson/")) {
      ...
      if (requestedDate === "latest") {
        const latestClosedDate = dayKey(new Date(Date.now() - 86400000));
        const latestClosed = parseClosedDate(latestClosedDate);
        if (!latestClosed) return json({ error: "lesson_not_available", earliest_date: launchDate, latest_closed_date: latestClosedDate }, 404);
        return json(closedLesson(latestClosedDate, latestClosed), 200, { "cache-control": "public, max-age=300" });
      }
      const date = parseClosedDate(requestedDate);
